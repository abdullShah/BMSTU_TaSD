...
void queuearray_push(queue_t *queue, char c_elem)
{
    ...
    if (queue->p_in == queue->p_out && *((char *) queue->p_in) != '\0')
...
{
    if (queue->p_out == queue->p_in && *((char *) queue->p_in) == '\0')
        printf("Очередь пуста!\n");
    else
    {
...
void sd_array(int n, int interval, times_t t1, times_t t2, times_t t3, times_t t4)
{
    queue_t queue1;
